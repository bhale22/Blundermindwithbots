...
async function botMakeMove() {
  ...
  // Game-generation token: botStart/botStop bump _botGameGen, so if the game
  // is restarted or stopped while an engine reply is in flight, every resume
  // point below sees the mismatch and discards the stale move instead of
  // executing it into the new game (the botActive check alone can't tell —
  // a restarted game sets it right back to true).
  const _myGen = _botGameGen;
  ...
  try {
    ...
    if (preferredOpeningActive && botOpeningMode === 'preferred') {
      ...
      if (!slots.length) {
        ...
        if (_myGen !== _botGameGen) return; // game restarted during ECO load
  ...
  // Stale check AFTER every engine await: a restart bumped the generation, so
  // this reply belongs to a dead game — leave the new game's state alone.
  if (_myGen !== _botGameGen) return;
